...
object Refs extends ZIOAppDefault {
  ...
  def demoConcurrentWorkImpure(): UIO[Unit] = {
    ...
    val effects = List("I love ZIO", "This Ref this is cool", "Vinh writes a lot of code!").map(task)
  ...
  def demoConcurrentWorkPure(): UIO[Unit] = {
    ...
    for {
      ...
      _ <- ZIO.collectAllParDiscard(
        List("I love ZIO", "This Ref this is cool", "Vinh writes a lot of code!")
  ...
  // 2 - always prints 0, because ticksRef is an EFFECT that CREATES A NEW REF
