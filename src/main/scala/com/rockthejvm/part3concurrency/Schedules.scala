...
object Schedules extends ZIOAppDefault {
  ...
  val aRetriedZIO = aZIO.retry(Schedule.recurs(10)) // tries 10 more times, returns the first success, last failure
  ...
  val fixedIntervalSchedule = Schedule.spaced(1.second) // retries every second until a success is returned

  // exponential backoff
  val exBackoffSchedule = Schedule.exponential(1.second, 2.0) // 1 - 2 - 4 - 8 - ... 2^n
  ...
  val totalElapsed = Schedule.spaced(1.second) >>> Schedule.elapsed.map(time => println(s"total elapsed: $time"))
